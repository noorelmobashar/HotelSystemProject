...
const currentRole = computed(() => String(page.props.auth?.role ?? '').toLowerCase());
const canManage = computed(() => ['admin', 'manager'].includes(currentRole.value));
const canApprove = computed(() => ['admin', 'manager', 'receptionist'].includes(currentRole.value));

const filterState = reactive({
    search: props.filters.search ?? '',
    per_page: Number(props.filters.per_page ?? 10),
    sort_by: props.filters.sort_by ?? 'approval_status',
...
const pagination = computed(() => props.clients ?? {
    data: [],
    current_page: 1,
    last_page: 1,
    per_page: 10,
...
const formatDate = (value) => value || 'Pending';
...
const columns = computed(() => [
    ...
    },
    {
        ...
        cell: ({ row }) => {
            ...
            return h('span', { class: className }, status === 'approved' ? 'Approved' : 'Pending');
        },
    },
    {
        header: 'Approved At',
        accessorKey: 'approved_at',
        meta: { sortKey: 'approved_at' },
...
const resetFilters = () => {
    ...
    filterState.sort_by = 'approval_status';
...
const approveClient = async (id) => {
    if (!window.confirm('Approve this client account now?')) {
        return;
    }

    await axios.patch(route('clients.approve', id), {}, {
        headers: {
...
const deleteClient = async (id) => {
    if (!window.confirm('Warning: this action permanently deletes the client account and related reservations. Continue?')) {
        return;
    }

    await axios.delete(route('clients.destroy', id), {
        headers: {
            Accept: 'application/json',
            'X-Requested-With': 'XMLHttpRequest',
        },
    ...
    <RoleDashboardLayout>
        <div class="h-full overflow-y-auto bg-gradient-to-br from-slate-100 via-slate-100 to-slate-200/60 px-6 py-8 lg:px-10">
            <div class="mx-auto max-w-7xl space-y-6">
                <div class="flex flex-col gap-4 rounded-3xl border border-slate-200 bg-white p-6 shadow-[0_18px_40px_-34px_rgba(15,23,42,0.95)] md:flex-row md:items-end md:justify-between">
                    <div>
                        ...
                        <p class="mt-1 text-sm text-slate-500">
                            Review guest accounts, approve pending clients, and manage client records.
                        </p>
                    </div>
                    ...
                    >
                        {{ showCreateForm ? 'Close Create Form' : 'Add Client' }}
                    </button>
                </div>
                ...
                >
                    <h2 class="text-lg font-semibold text-slate-800">Create Client</h2>
                ...
                <div
                    v-if="canManage && editForm.id"
                    class="rounded-3xl border border-cyan-200 bg-white p-6 shadow-[0_18px_40px_-34px_rgba(15,23,42,0.95)]"
                >
                    <h2 class="text-lg font-semibold text-slate-800">Edit Client</h2>
                        ...
                        <div class="grid gap-3 sm:grid-cols-2">
                            <div>
                                ...
                                <input
                                    ...
                                    placeholder="Name, email, national ID, gender"
                                >
                            </div>
                            <div>
                            ...
                            <tbody class="divide-y divide-slate-200 bg-white">
                                ...
                                >
                                    ...
                                    <td class="px-4 py-3 text-sm">
                                        <div class="flex flex-wrap gap-2">
                                            <button
                                                v-if="canApprove && row.original.status === 'pending'"
                                                type="button"
                                                class="rounded-lg border border-emerald-300 px-3 py-1.5 text-xs font-semibold text-emerald-700 transition hover:bg-emerald-50"
                                            ...
                                            <button
                                                v-if="canManage"
                                                type="button"
                                                class="rounded-lg border border-cyan-300 px-3 py-1.5 text-xs font-semibold text-cyan-700 transition hover:bg-cyan-50"
                                ...
                                <tr v-if="table.getRowModel().rows.length === 0">
                                    <td colspan="8" class="px-4 py-8 text-center text-sm text-slate-500">
                                        {{ canManage ? 'No clients found.' : 'No pending clients found.' }}
                                    </td>
                                </tr>
                            </tbody>
                        </table>
