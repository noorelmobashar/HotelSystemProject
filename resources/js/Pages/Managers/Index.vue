...
const pagination = computed(() => props.managers ?? {
    data: [],
    current_page: 1,
    last_page: 1,
    per_page: 10,
...
const submitEdit = () => {
    editForm.put(route('managers.update', editForm.id), {
        preserveScroll: true,
        onSuccess: () => {
            cancelEdit();
        },
...
const deleteManager = async (id) => {
    if (!window.confirm('Warning: this action permanently deletes the manager account. Continue?')) {
        return;
    }

    await axios.delete(route('managers.destroy', id), {
        headers: {
        ...
        <div class="h-full overflow-y-auto bg-gradient-to-br from-slate-100 via-slate-100 to-slate-200/60 px-6 py-8 lg:px-10">
            <div class="mx-auto max-w-7xl space-y-6">
                <div class="flex flex-col gap-4 rounded-3xl border border-slate-200 bg-white p-6 shadow-[0_18px_40px_-34px_rgba(15,23,42,0.95)] md:flex-row md:items-end md:justify-between">
                    <div>
                        <h1 class="text-2xl font-semibold text-slate-900">Manage Managers</h1>
                        <p class="mt-1 text-sm text-slate-500">
                            Create, update, and remove property manager accounts.
                        </p>
                    </div>
                    ...
                    >
                        {{ showCreateForm ? 'Close Create Form' : 'Add Manager' }}
                    </button>
                </div>
                ...
                >
                    <h2 class="text-lg font-semibold text-slate-800">Create Manager</h2>
                ...
                <div
                    v-if="editForm.id"
                    class="rounded-3xl border border-sky-200 bg-white p-6 shadow-[0_18px_40px_-34px_rgba(15,23,42,0.95)]"
                >
                    <h2 class="text-lg font-semibold text-slate-800">Edit Manager</h2>
                ...
                <div class="rounded-3xl border border-slate-200 bg-white p-6 shadow-[0_18px_40px_-34px_rgba(15,23,42,0.95)]">
                                ...
                                >
                                    ...
                                    <td class="px-4 py-3 text-sm">
                                        <div class="flex gap-2">
                                            <button
                                                type="button"
                                                class="rounded-lg border border-sky-300 px-3 py-1.5 text-xs font-semibold text-sky-700 transition hover:bg-sky-50"
                                            ...
                                            <button
                                                ...
                                                @click="deleteManager(row.original.id)"
                                            >
                                                Delete
                                            </button>
                                        </div>
                    ...
                    <div class="mt-4 flex flex-col gap-3 text-sm text-slate-500 md:flex-row md:items-center md:justify-between">
                        <p>
                            Showing {{ pagination.from ?? 0 }} to {{ pagination.to ?? 0 }} of {{ pagination.total ?? 0 }} managers
                        </p>
